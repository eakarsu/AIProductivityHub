...
// User\'s email, calendar, Slack, tasks; AI synthesizes into unified workday view; suggests optimal task ordering, meeting scheduling, break timing
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductivityHub / cf-cross-tool-optimization',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_cross_tool_optimization`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-cross-tool-optimization feature
router.post('/run', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Cross-tool optimization'.
Context: User\'s email, calendar, Slack, tasks; AI synthesizes into unified workday view; suggests optimal task ordering, meeting scheduling, break timing
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-cross-tool-optimization/run error:', err);
