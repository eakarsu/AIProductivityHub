...
// Monitor computer activity; AI detects when user is distracted; send gentle nudge with refocus suggestion
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductivityHub / cf-distraction-detector',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_distraction_detector`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-distraction-detector feature
router.post('/run', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Distraction detector'.
Context: Monitor computer activity; AI detects when user is distracted; send gentle nudge with refocus suggestion
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-distraction-detector/run error:', err);
