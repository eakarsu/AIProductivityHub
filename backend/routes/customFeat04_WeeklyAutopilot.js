...
// Continuous background tracking of all activities; AI generates weekly digest (goals, habits, focus, usage); highlights successes and areas for improvement
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductivityHub / cf-weekly-autopilot',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_weekly_autopilot`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-weekly-autopilot feature
router.post('/run', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Weekly autopilot'.
Context: Continuous background tracking of all activities; AI generates weekly digest (goals, habits, focus, usage); highlights successes and areas for improvement
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-weekly-autopilot/run error:', err);
