...
// User sets goal; AI breaks into milestones; auto-adjusts timeline based on progress; recommends focus time, task decomposition, habit supports
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductivityHub / cf-agentic-goal-orchestration',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_goal_orchestration`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-goal-orchestration feature
router.post('/run', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic goal orchestration'.
Context: User sets goal; AI breaks into milestones; auto-adjusts timeline based on progress; recommends focus time, task decomposition, habit supports
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-goal-orchestration/run error:', err);
