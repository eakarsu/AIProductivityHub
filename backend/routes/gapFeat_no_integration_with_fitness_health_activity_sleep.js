...
// Gap Feature: No integration with fitness/health (activity, sleep)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductivityHub / gap-no-integration-with-fitness-health-activity-sleep',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No integration with fitness/health (activity, sleep)'.\nProject: AIProductivityHub.\nContext: No integration with fitness/health (activity, sleep)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-integration-with-fitness-health-activity-sleep', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-integration-with-fitness-health-activity-sleep', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
