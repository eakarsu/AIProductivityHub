...
// Gap (AI): Goals without '/goal
// Goals without /goal-progress-predict (predict likelihood of goal completion)
import React, { useState } from 'react';

export default function GapGoalsWithoutGoalPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-goals-without-goal/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Goals without '/goal</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Goals without /goal-progress-predict (predict likelihood of goal completion)</p>
