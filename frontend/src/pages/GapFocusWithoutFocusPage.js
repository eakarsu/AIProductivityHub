...
// Gap (AI): Focus without '/focus
// Focus without /focus-boost-recommend (suggest focus optimization techniques)
import React, { useState } from 'react';

export default function GapFocusWithoutFocusPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-focus-without-focus/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Focus without '/focus</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Focus without /focus-boost-recommend (suggest focus optimization techniques)</p>
