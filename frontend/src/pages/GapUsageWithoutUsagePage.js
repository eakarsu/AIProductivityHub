...
// Gap (AI): Usage without '/usage
// Usage without /usage-anomaly-detect (flag unusual behavior patterns)
import React, { useState } from 'react';

export default function GapUsageWithoutUsagePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-usage-without-usage/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Usage without '/usage</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Usage without /usage-anomaly-detect (flag unusual behavior patterns)</p>
