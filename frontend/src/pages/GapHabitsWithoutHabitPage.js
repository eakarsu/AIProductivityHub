...
// Gap (AI): Habits without '/habit
// Habits without /habit-auto-adjust (difficulty adaptation)
import React, { useState } from 'react';

export default function GapHabitsWithoutHabitPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-habits-without-habit/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Habits without '/habit</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Habits without /habit-auto-adjust (difficulty adaptation)</p>
