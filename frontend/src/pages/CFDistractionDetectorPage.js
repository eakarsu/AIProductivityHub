...
// Custom Feature: Distraction detector
// Distraction detector: Monitor computer activity; AI detects when user is distracted; send gentle nudge with refocus suggestion
import React, { useState } from 'react';

export default function CFDistractionDetectorPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-distraction-detector/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Distraction detector</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Distraction detector: Monitor computer activity; AI detects when user is distracted; send gentle nudge with refocus suggestion</p>
