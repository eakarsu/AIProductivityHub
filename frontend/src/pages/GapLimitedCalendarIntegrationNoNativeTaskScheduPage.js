...
// Gap (Non-AI): Limited calendar integration (no native task scheduling sync)
// Limited calendar integration (no native task scheduling sync)
import React, { useState } from 'react';

export default function GapLimitedCalendarIntegrationNoNativeTaskScheduPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-calendar-integration-no-native-task-schedu/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Limited calendar integration (no native task scheduling sync)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Limited calendar integration (no native task scheduling sync)</p>
